...
class SparkProductionApp:
    ...
    def run_etl_pipeline(self, input_path: str, output_path: str, date: str):
        """Execute ETL pipeline"""
        logger.info("starting_etl_pipeline", input_path=input_path, date=date)
        if not self.spark:
            raise RuntimeError("Spark session not initialized. Call initialize_spark() first.")
        try:
            with self.metrics.timer("etl_pipeline_duration"):
                ...
                logger.info("etl_pipeline_completed", 
    ...
    def run_streaming_pipeline(self, kafka_config: dict, checkpoint_path: str):
    ...
    def run_ml_pipeline(self, training_data_path: str, model_path: str):
...
@click.command()
@click.option('--mode', type=click.Choice(['etl', 'streaming', 'ml', 'all']), 
              default='etl', help='Pipeline mode to run')
...
@click.option('--checkpoint-path', default='checkpoint/', 
              help='Streaming checkpoint path')
@click.option('--model-path', default='models/', help='ML model path')
@click.option('--config-file', type=click.Path(exists=True), 
              help='Configuration file path')
def main(mode, input_path, output_path, date, kafka_servers, kafka_topic, 
         checkpoint_path, model_path, config_file):
    """
