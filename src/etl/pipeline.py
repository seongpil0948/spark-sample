...
class ETLPipeline:
    ...
    def define_schema(self) -> StructType:
        ...
        return StructType(
            [
                ...
                ),  # Allow nulls for data quality testing
    ...
    def run_quality_checks(self, input_path: str, date: str) -> Dict[str, Any]:
        ...
        latest_timestamp = df.agg(spark_max("timestamp").alias("max_ts")).collect()[0][
            "max_ts"
        ]
    ...
    def _calculate_quality_score(
        self, total: int, nulls: Dict, duplicates: int, outliers: int
    ) -> float:
    ...
    def transform_data(self, input_path: str, date: str) -> DataFrame:
        ...
        df = (
            self.spark.read.schema(self.define_schema())
        ...
        # Feature engineering
        enriched_df = (
            cleaned_df.withColumn("processed_date", current_date())
            ...
            .withColumn(
                "tag_count",
                when(col("tags").isNotNull(), size(col("tags"))).otherwise(0),
            )
            .withColumn(
                "has_properties",
                when(
                    col("properties").isNotNull() & (size(col("properties")) > 0), True
                ).otherwise(False),
            )
        )

        # Business logic transformations with Spark 4.0 filter hints
        final_df = enriched_df.hint("predicate_pushdown").filter(
    ...
    def generate_summary(self, df: DataFrame) -> DataFrame:
        ...
        # Hourly aggregations
        hourly_summary = df.groupBy("hour", "event_type", "amount_category").agg(
            ...
            spark_sum(when(col("has_properties"), 1).otherwise(0)).alias(
                "events_with_properties"
            ),
        )

        # Calculate conversion metrics
        summary_with_metrics = hourly_summary.withColumn(
            "revenue_per_user",
            when(
                col("unique_users") > 0, col("total_amount") / col("unique_users")
            ).otherwise(0),
        ).withColumn(
            "properties_usage_rate",
            when(
                col("event_count") > 0,
                col("events_with_properties") / col("event_count"),
            ).otherwise(0),
        )
    ...
    def save_results(
        ...
        transformed_df.write.mode("overwrite").option("compression", "snappy").option(
            "maxRecordsPerFile", "1000000"
        ).option("spark.sql.files.maxPartitionBytes", "128MB").partitionBy(
            "processed_date", "hour"
        ).parquet(f"{output_path}/detailed/date={date}/")

        # Save summary data with Spark 4.0 adaptive coalescing
        summary_df.coalesce(1).sortWithinPartitions("hour", "event_type").write.mode(
            "overwrite"
        ).option("compression", "snappy").parquet(f"{output_path}/summary/date={date}/")

        # Generate and save daily rollup
        daily_rollup = summary_df.groupBy("event_type", "amount_category").agg(
        ...
        daily_rollup.coalesce(1).write.mode("overwrite").parquet(
            f"{output_path}/daily_rollup/date={date}/"
        )

        logger.info(
            "results_saved_successfully",
