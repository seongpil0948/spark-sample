...
from pyspark.sql.functions import (
    ...
    collect_list, expr, concat_ws, split, explode,
    when, coalesce, date_format, hour, minute, lit
)
...
class StreamingProcessor:
    ...
    def process_kafka_stream(self, bootstrap_servers: str, topic: str,
                           checkpoint_path: str, output_path: str) -> StreamingQuery:
        """Process Kafka stream with watermarking and windowing"""
        logger.info("starting_kafka_stream_processing", 
        ...
        aggregation_query = windowed_aggregations.writeStream \
            .outputMode("append") \
            .format("delta") \
        ...
        detail_query = enriched_stream.writeStream \
            .outputMode("append") \
            .format("delta") \
    ...
    def _setup_alerting(self, df: DataFrame, checkpoint_path: str):
        """Setup real-time alerting for anomalies"""
        # High-value transaction alerts
        high_value_alerts = df.filter(col("amount") > 10000) \
            .select(
                col("event_id"),
                col("user_id"),
                col("amount"),
        ...
        alert_query = high_value_alerts.writeStream \
            .outputMode("append") \
            .format("json") \
            .option("checkpointLocation", f"{checkpoint_path}/alerts") \
            .option("path", "alerts/high_value") \
            .trigger(processingTime="5 seconds") \
            .start()
        
        logger.info("alerting_system_initialized")
        
    def process_with_stateful_operations(self, bootstrap_servers: str, 
                                        topic: str, checkpoint_path: str) -> StreamingQuery:
        ...
        sessionized = events \
            ...
            .groupBy(
                col("user_id"),
                window(col("timestamp"), "20 minutes", "5 minutes")
            ) \
            .agg(
        ...
        pattern_detection = sessionized \
            .withColumn("has_purchase_pattern",
                expr("array_contains(event_sequence, 'VIEW') AND " +
                     "array_contains(event_sequence, 'ADD_TO_CART') AND " +
                     "array_contains(event_sequence, 'PURCHASE')")
            ) \
            .filter(col("has_purchase_pattern") == True)
        
        # Write results
        query = pattern_detection.writeStream \
            .outputMode("update") \
            .format("delta") \
    ...
    def create_streaming_ml_pipeline(self, model_path: str, 
                                   bootstrap_servers: str, 
                                   input_topic: str,
                                   output_topic: str) -> StreamingQuery:
        ...
        requests = self.spark.readStream \
            .format("kafka") \
            .option("kafka.bootstrap.servers", bootstrap_servers) \
