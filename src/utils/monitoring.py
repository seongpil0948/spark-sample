...
class MetricsCollector:
    ...
    def increment(self, metric_name: str, labels: Optional[Dict[str, str]] = None, 
        ...
        try:
            if metric_name == "pipeline_runs":
                self.pipeline_runs.labels(**labels).inc(value)
            elif metric_name == "records_processed":
                self.records_processed.labels(**labels).inc(value)
            elif metric_name == "errors":
                self.errors.labels(**labels).inc(value)
            else:
                logger.warning("unknown_metric", metric=metric_name)
        except Exception as e:
            logger.error("metric_increment_failed", 
                        metric=metric_name, 
                        error=str(e))
    ...
    def set_gauge(self, metric_name: str, value: float, 
        ...
        try:
            ...
            logger.error("gauge_set_failed", 
                        metric=metric_name, 
                        error=str(e))
    
    def record_data_quality(self, dataset: str, score: float):
    ...
    def collect_spark_metrics(self, spark_context):
        ...
        try:
            ...
            if hasattr(status, 'getExecutorInfos'):
                ...
                self.set_gauge("memory_usage", used_memory, 
                             labels={"component": "executors"})
...
class PerformanceMonitor:
    ...
    def record_operation(self, operation: str, start_time: datetime, 
                        end_time: datetime, records: int, 
    ...
    def _analyze_performance(self, operation: str):
        ...
        if len(operation_metrics) >= 5:
            ...
            if latest_duration > avg_duration * 1.5:
                logger.warning("performance_degradation_detected",
                             operation=operation,
